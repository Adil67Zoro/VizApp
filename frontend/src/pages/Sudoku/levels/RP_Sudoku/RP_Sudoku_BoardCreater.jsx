...
import '../../Sudoku.css'
import { initialBoard, SudokuPosition } from "../../../Chess/Constants";

export default function RP_Sudoku_BoardCreater({xdim, ydim, InitialPosition}){
    ...
    for (let y=ydim-1; y>=0; y--){
        for (let x=0; x<xdim; x++){
            ...
            initialPieces.push(<Squares key={[x, y]} xpos={x} ypos={y} sudoku={true} dimension={100} number={value} chosen={chosen} length={xdim} />)
        }
    }
        ...
        <div className={className} ref={sudokuBoardRef}
            ...
            {openSuccess && (
                <div className="success">
                    ...
                    navigate(`/sudoku/RecreatePosition/${xdim}x${ydim}/${nextLevel}`)
                    restartBoards()
                    }}>{'Next Level ->'}</button>
                </div>)}
            {initialPieces}
